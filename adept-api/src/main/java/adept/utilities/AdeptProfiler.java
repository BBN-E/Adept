...
public abstract class AdeptProfiler
{
	...
	public static void initialize(String filename, String outputDir)
	{
		...
		try {
			...
			test_summary_writer.write("Config File Name\tActivation Time\tTotal File Count\tFailure Count\tTotal Process Time\tAverage File Size\tMax File Size\tMin File Size\tFile Size Variance\tAvg Char Count\tMax Char Count\tMin Char Count\tChar Count Variance\tAvg Process Time (s)\tMax Process Time\tMin Process Time\tProcess Time Variance\tAvg Heap Usage\tMax Heap Usage\tMin Heap Usage\tHeap Usage Variance\t(Final Heap - Initial Heap)\n");
	...
	public void Run(String packagePathIn, String configFilenameIn, String profileFilename, String outputDir)
	{
		...
		for(File file : dataDirectory.listFiles()) {
				...
				if ( bFirst ) 
				{
				   ...
				   try {
						...
						document_html_writer.write("<!DOCTYPE html>\n<html>\n<head>\n<style>\ntable,th,td\n{\nfont-family: Calibri, sans-serif;\nborder-collapse: collapse;\nborder:1px solid black;\nfont-size: 10pt;\n}\ntd\n{\ntext-align:right;\n}\ntable\n{\nwidth: 100%;\n}\n</style>\n</head>\n<body>\n<h2>" + configFilename.replace("ProfilerConfig.xml" , "") + " Profiling Results, " + dateFormat.format(date) + "</h2>\n<table>\n");

						document_html_writer.write("<tr><th>Doc name</th><th>Document Size(Bytes)</th><th>Num of Characters</th><th>Num of Passages</th><th>Number of Tokens</th><th>Process time(seconds)</th><th>Heap usage(MB)</th></tr>\n");
            ...
            if(totalFiles % summarySize == 0)
            {
                try {
                    ...
                    test_summary_writer.write(configFilename + "\t" + activate.elapsedTime(TimeUnit.MILLISECONDS)/1000.0 + "\t" +
                                              ...
                                              + calculateAverage(processTimes) + "\t" + calculateMax(processTimes) + "\t" + calculateMin(processTimes) + "\t"
                                              + calculateVariance(processTimes) + "\t" + calculateAverage(heapUsages) + "\t" + calculateMax(heapUsages) + "\t"
                                              + calculateMin(heapUsages) + "\t" + calculateVariance(heapUsages) + "\t" + (heapUsages.get(heapUsages.size()-1) - heapUsages.get(0)) + "\n");
		...
		try {
			...
			document_html_writer.write("</table>\n</body>\n</html>");
			...
			machine_stats_writer.write(machineMemory + "\t" + machineCores + "\t" + osName + "\t"
			...
			test_summary_writer.close();
                	...
                	test_html_writer.write("<tr><td>" + activate.elapsedTime(TimeUnit.MILLISECONDS)/1000.0 + "</td><td>" +
                                              ...
                                              + String.format("%.2f", calculateAverage(processTimes)) + "</td><td>" + String.format("%.2f", calculateMax(processTimes)) + "</td><td>" + String.format("%.2f", calculateMin(processTimes)) + "</td><td>"
                                              + String.format("%.2f", calculateVariance(processTimes)) + "</td><td>" + String.format("%.2f", calculateAverage(heapUsages)) + "</td><td>" + String.format("%.2f", calculateMax(heapUsages)) + "</td><td>"
                                              + String.format("%.2f", calculateMin(heapUsages)) + "</td><td>" + String.format("%.2f", calculateVariance(heapUsages)) + "</td><td>" + String.format("%.2f", (heapUsages.get(heapUsages.size()-1) - heapUsages.get(0))) + "</td></tr>\n");
			
			test_html_writer.write("</table><p>\n<a href=\"document_stats.html\">Full Document Statistics</a>\n<p><img src=\"ProcessTime_vs_FileSize.png\">\n<p><img src=\"ProcessTime_vs_NumberOfTokens.png\">\n<p><img src=\"ProcessTime_vs_NumberOfPassages.png\">\n<p><img src=\"HeapMemoryUsage.png\">\n</body>\n</html>");
		...
		// graph making
		try {
			BufferedReader br = new BufferedReader(new FileReader(document_stats.getAbsolutePath()));
            for(int j=0;j<5;j++)
            {
            	br.readLine();
            }
			...
			Grapher.makeTimeVsSizeGraph(processTimes, fileSizes, new File(outputDir+"/ProcessTime_vs_FileSize.png"), "Profiling Corpus", "File Size(Bytes)", "Process Time vs. File Size", "Process Time (s)");
			Grapher.makeTimeVsSizeGraph(processTimes, numOfPassages, new File(outputDir+"/ProcessTime_vs_NumberOfPassages.png"), "Profiling Corpus", "Num of Passages", "Process Time vs. Passage Count", "Process Time (s)");
			Grapher.makeTimeVsSizeGraph(processTimes, numOfTokens, new File(outputDir+"/ProcessTime_vs_NumberOfTokens.png"), "Profiling Corpus", "Num Of Tokens", "Process Time vs. Token Count", "Process Time (s)");
	 ...
	 */
	private boolean processInputFile( File file, Double[] docStatistics, HltContentContainer hltcontainer) {
        ...
        if(document != null)
		{
			...
			if(hltcontainer==null) {
				...
				try
				{
					String serialized = xmlserializer.serializeAsString(hltcontainer);
